...
# Проверка на root
if [ "$EUID" -ne 0 ]; then
    log "❌ Пожалуйста, запустите скрипт с sudo"
    exit 1
fi
...
# Создание bot.py
log "[*] Создание бота: /opt/fail2ban-telegram-bot/bot.py"
cat << 'EOF' > /opt/fail2ban-telegram-bot/bot.py
#!/usr/bin/env python3
import subprocess
import re
import logging
...
cat << 'EOF' > /etc/profile.d/ssh-notify.sh
#!/bin/bash
...
log "[*] Создание Fail2Ban action для Telegram..."
mkdir -p /etc/fail2ban/action.d
cat << 'EOF' > /etc/fail2ban/action.d/telegram.conf
[Definition]

actionban = /usr/local/bin/fail2ban-telegram-alert.sh ban <ip> <name> <failures>

actionunban = /usr/local/bin/fail2ban-telegram-alert.sh unban <ip> <name>
...
log "[*] Создание скрипта уведомлений Fail2Ban..."
cat << 'EOF' > /usr/local/bin/fail2ban-telegram-alert.sh
#!/bin/bash
...
log "[*] Настройка Fail2Ban jail.local..."
...
log "[*] Перезагрузка systemd и включение сервисов..."
